...
Shader "PRIM3/Outline/Post Outline"
{
     Properties
    {
    ...
    SubShader 
    {
        ...
        Pass 
        {
            ...
            half4 frag(v2f i) : COLOR 
            {
                ...
                {
                    //increase our output color by the pixels in the area
                    ColorIntensityInRadius += tex2D(_GrabTexture, float2(i.uvs.x, 1 - i.uvs.y) + float2(0, (j-NumberOfIterations/2)*TX_y))/NumberOfIterations;
                }
 
 
                //this is alpha blending, but we can't use HW blending unless we make a third pass, so this is probably cheaper.
                half4 outcolor = _Intensity * ColorIntensityInRadius  * 2 + (1-ColorIntensityInRadius) * tex2D(_SceneTex, i.uvs);
                return outcolor;
            }
