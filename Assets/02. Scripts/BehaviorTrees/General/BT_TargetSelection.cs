...
[TaskDescription("Encuentra y almacena un objetivo para el enemigo.")]
[TaskCategory("Enemies")]
public class BT_TargetSelection : Action {
	public SharedGameObject target;
	public SharedGameObject self;
	[Header("Parametros de seleccion")]
	public SharedFloat range = 50.0f;
	public SharedFloat rangeToStop = 60.0f;
	...
	public override void OnAwake ()
	{
		...
		DebugDrawer.AddOval (0f, 0f, range.Value, range.Value, Color.red, gameObject);
		DebugDrawer.AddOval (0f, 0f, rangeToStop.Value, rangeToStop.Value, Color.magenta, gameObject);
	}
	...
	public override TaskStatus OnUpdate ()
	{
		...
		if (!currentTarget.target && currentTarget.player != -1) {
			...
			target.Value = currentTarget.target;
		}
	...
	TargetPoints SelectTarget() {
		...
		for (int i = 0; i < players.childCount; ++i) {
			...
			if (validTarget) {
				...
				if (distanceFromPlayer < range.Value) {
				...
				} else if (distanceFromPlayer > rangeToStop.Value && playerNum == currentTarget.player) {
	...
	float ComputeDistance2D(float distance) {
		return pDistance2DToTarget - ((distance / range.Value) * pDistance2DToTarget);
	}
