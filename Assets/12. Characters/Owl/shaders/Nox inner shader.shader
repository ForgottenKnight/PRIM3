...
Shader "Custom/Outline and ScreenSpace texture"
{
	Properties
	{
		...
		_RefractionFactor ("Refracción", Range(0,1)) = 0
	}
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float2 screenVector = (i.pos.xy / _ScreenParams.xy);
				float2 xy = (normalize(i.normal.xy) - normalize(screenVector)) * _RefractionFactor;
				return tex2D(_MainTex, ((i.pos.xy / _ScreenParams.xy) + xy + float2(_Time.y * _SpeedX, _Time.y * _SpeedY)) / _Zoom);
				//return tex2D(_MainTex, ((i.pos.xy / _ScreenParams.xy) + float2(_Time.y * _SpeedX, _Time.y * _SpeedY)) / _Zoom);
			}
